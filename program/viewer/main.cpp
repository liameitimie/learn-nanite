...
int main(){
    ...
    fmt::print("cluster triangles: {} us\n",timer.us());

    timer.reset();
    group_clusters(clusters,0,clusters.size(),cluster_groups);
    fmt::print("group clusters: {} us\n",timer.us());
    ...
    u32 c_id=0;
    for(auto& cluster:clusters){
        u32 g_id=cluster.group_id;
        bool b=false;
        for(u32 x:cluster_groups[g_id].clusters){
            if(x==c_id) b=true;
        }
        assert(b);
        c_id++;
    ...
    for(auto& cluster:clusters){
        ...
        packed_data.push_back(cluster.group_id); //group id
        packed_data.push_back(0); //padding
    ...
    while(!window.should_close()){
        ...
        FrameContext frame_context{vp_mat,view_mode};
